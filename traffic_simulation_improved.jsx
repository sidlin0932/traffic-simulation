...
function stepLightsAdaptive(g, state, mode, revMode, oneWayPairs) {
  ...
  return lights.map((row, r) => row.map((l, c) => {
    ...
    for (let i = 0; i <= SENSOR_RANGE; i++) {
      const fwdCell = isHFwdReversed(r) ? (hm - i) : (hp - i);
      const bwdCell = isHBwdReversed(r) ? (hp - i) : (hm - i);
      if (fwdCell >= 0 && hFwd[r][fwdCell]) hCars++;
      if (bwdCell >= 0 && hBwd[r][bwdCell]) hCars++;
...
// Custom route planning based on direction constraints
...
function clearIntersections(g, state, rng, revMode, oneWayPairs) {
  const { hFwd, hBwd, vFwd, vBwd, lights } = state;
  ...
  for (let r = 0; r < NUM_H; r++) {
    for (let c = 0; c < NUM_V; c++) {
      ...
      const fwdCell = isHFwdReversed(r) ? hm : hp;
      const bwdCell = isHBwdReversed(r) ? hp : hm;

      if (!hg) {
        if (free(hFwd[r], fwdCell) && !tryForward(hFwd[r], fwdCell)) tryForceTurn(hFwd[r], fwdCell, vFwd[c], vp, vBwd[c], vm);
        if (free(hBwd[r], bwdCell) && !tryForward(hBwd[r], bwdCell)) tryForceTurn(hBwd[r], bwdCell, vFwd[c], vp, vBwd[c], vm);
      }
      if (hg) {
        if (free(vFwd[c], vp) && !tryForward(vFwd[c], vp)) tryForceTurn(vFwd[c], vp, hFwd[r], fwdCell, hBwd[r], bwdCell);
        if (free(vBwd[c], vm) && !tryForward(vBwd[c], vm)) tryForceTurn(vBwd[c], vm, hFwd[r], fwdCell, hBwd[r], bwdCell);
      }
    }
  }
    ...
    for (let r = 0; r < NUM_H; r++) {
      for (let c = 0; c < NUM_V; c++) {
        ...
        if (g.tjunc[r][c] === "down" && vFwd[c][vp] && !vFwd[c][vp].routed)
          tryForceTurn(vFwd[c], vp, hFwd[r], fwdCell, hBwd[r], bwdCell);
        if (g.tjunc[r][c] === "up" && vBwd[c][vm] && !vBwd[c][vm].routed)
          tryForceTurn(vBwd[c], vm, hFwd[r], fwdCell, hBwd[r], bwdCell);
      }
    }
  }
}
...
function processTurns(g, state, turnProb, rng, revMode, oneWayPairs) {
  ...
  const getTurnTargets = (r, c, vp, vm) => {
    ...
    if (bwdReversed) {
      ...
      tE = { road: hFwd[r], ti: mirror(g.hInt[c], g.HLEN) }; // hFwd is Westbound
    }
    return [tE, tW];
  };
  ...
  for (let r = 0; r < NUM_H; r++) {
    for (let c = 0; c < NUM_V; c++) {
      ...
      const hg = lights[r][c].hGreen;
      if (hg) {
        if (hFwd[r][hp] && !hFwd[r][hp].routed && rng() < turnProb) turns.push({ src: hFwd[r], si: hp, targets: [{ road: vFwd[c], ti: vp }, { road: vBwd[c], ti: vm }] });
        if (hBwd[r][hm] && !hBwd[r][hm].routed && rng() < turnProb) turns.push({ src: hBwd[r], si: hm, targets: [{ road: vFwd[c], ti: vp }, { road: vBwd[c], ti: vm }] });
      } else {
        if (vFwd[c][vp] && !vFwd[c][vp].routed && rng() < turnProb) turns.push({ src: vFwd[c], si: vp, targets: getTurnTargets(r, c, vp, vm) });
        if (vBwd[c][vm] && !vBwd[c][vm].routed && rng() < turnProb) turns.push({ src: vBwd[c], si: vm, targets: getTurnTargets(r, c, vp, vm) });
      }
    }
  }
...
function processRoutes(g, state, rng, revMode, oneWayPairs) {
  ...
  const getTargetRoad = (heading, idx, crossCell) => {
    ...
    if (heading === "E") {
      roads.push(hFwd[idx]);
      if (bwdReversed) roads.push(hBwd[idx]);
    } else if (heading === "W") {
      roads.push(hBwd[idx]);
      if (fwdReversed) roads.push(hFwd[idx]);
    } else if (heading === "S") {
      roads.push(vFwd[idx]);
    } else {
      roads.push(vBwd[idx]);
    ...
    for (const rd of roads) {
      ...
      const w = wallOf.get(rd);
      const blocked = (w && (w.has(landing) || w.has(crossCell))) ||
        !(landing < rd.length && rd[landing] === null && rd[crossCell] === null);
      if (!blocked) return rd;
    }
    return roads[0];
  };
  ...
  const cand = [];
  const scanRoad = (road, heading, idx) => {
    for (let i = 0; i < road.length; i++) {
      const car = road[i];
  ...
  for (let r = 0; r < NUM_H; r++) {
    scanRoad(hFwd[r], isHFwdReversed(r) ? "W" : "E", r);
    scanRoad(hBwd[r], isHBwdReversed(r) ? "E" : "W", r);
  }
  for (let c = 0; c < NUM_V; c++) {
    scanRoad(vFwd[c], "S", c);
    scanRoad(vBwd[c], "N", c);
  ...
  for (const t of cand) {
    ...
    const landing = crossCell + 1;
    const tRoad = getTargetRoad(out, out === "E" || out === "W" ? r : c, crossCell);
...
function stepSim(g, state, pSlow, turnProb, rng, pInject, opts) {
  ...
  const revMode = (opts && opts.reversibleMode) || "none";
  const oneWayPairs = !!(opts && opts.oneWayPairs);
  const nl = stepLightsAdaptive(g, state, mode, revMode, oneWayPairs);
  ...
  if (routed) {
    ...
    if (meterP > 0) {
      const injectMix = (rd, boundary, laneIdx, wall) => {
        if (wall && (wall.has(0) || wall.has(1))) return;
      ...
      res.vFwd.forEach((rd, c) => injectMix(rd, "N", c, vWallF[c]));
      res.vBwd.forEach((rd, c) => injectMix(rd, "S", c, vWallB[c]));
    }
  } else {
    res.hFwd.forEach((rd, r) => {
      if (oneWayPairs && r % 2 === 1) return;
...
function drawState(canvas, g, state, trackedCarRef, revMode, oneWayPairs) {
  ...
  for (let r = 0; r < NUM_H; r++) {
    ...
    hFwd[r].forEach((car, i) => {
      if (car) {
        const x = isHFwdReversed(r) ? (PAD + (g.HLEN - 1 - i) * C) : (PAD + i * C);
        drawCar(x, y1, car.v, car);
      }
    });
    hBwd[r].forEach((car, i) => {
      if (car) {
        const x = isHBwdReversed(r) ? (PAD + i * C) : (PAD + (g.HLEN - 1 - i) * C);
        drawCar(x, y0, car.v, car);
      }
    });
  }
  for (let c = 0; c < NUM_V; c++) {
  ...
  if (trackedCarRef && allRouted.length > 0) {
    if (!trackedCar) {
      ...
      if (trackedCar) trackedCarRef.current = trackedCar.car.id;
    }
...
const REVERSIBLE_MODES = [
  { id: "none", label: "Normal Lanes" },
  { id: "eastbound_peak", label: "Tidal Peak (Eastbound →)" },
  { id: "westbound_peak", label: "Tidal Peak (← Westbound)" },
];
...
export default function TrafficSim() {
  ...
  const redraw = useCallback(() => setStats(drawState(canvasRef.current, geoRef.current, simRef.current, trackedCarRef, reversibleMode, oneWayPairs)), [reversibleMode, oneWayPairs]);
  ...
  const exportCSV = () => {
    ...
    const headers = [
      ...
      "D-T Throughput",
      "Average Speed",
      "Average Travel Time (ticks)",
      "Arrival Rate (cars/tick)",
      "Completion Rate"
    ];
    ...
    const rows = [
      [
        ...
        a.tputVF.toFixed(4),
        a.tputVB.toFixed(4),
        a.avgSpeed.toFixed(4),
        a.avgTravelTime.toFixed(4),
        a.arrivalRate.toFixed(4),
      ...
      [
        ...
        b.tputVF.toFixed(4),
        b.tputVB.toFixed(4),
        b.avgSpeed.toFixed(4),
        b.avgTravelTime.toFixed(4),
        b.arrivalRate.toFixed(4),
        (b.completion * 100).toFixed(2) + "%"
      ]
    ];
    ...
    link.setAttribute("download", `experiment_results_seed_${seed}.csv`);
  ...
  if (routed) {
    sliders.push(["Compliance", complianceRate, 0, 100, (e) => setComplianceRate(+e.target.value), complianceRate + "%"]);
  }
    ...
    <div style={{
      ...
      <div style={{ textAlign: "center" }}>
        <h1 style={{ fontSize: "17px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "#8888a8", margin: 0 }}>
          NaSch Bidirectional Grid (Improved)
        </h1>
        <p style={{ fontSize: "10px", color: "#484860", margin: "2px 0 0" }}>
          dual-lane · dynamic compliance · reversible lane · one-way pairs · green wave · taipei scenario presets
        </p>
      </div>
      ...
      {routed && (
        <div style={{ display: "flex", gap: "16px", fontSize: "11px", color: "#606078", flexWrap: "wrap", justifyContent: "center" }}>
          <span style={{ color: "#c084fc" }}>routed (mix):</span>
          <span>compliance <b style={{ color: "#e9d5ff" }}>{complianceRate}%</b></span>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", justifyContent: "center", alignItems: "center", background: "#111122", padding: "6px 12px", borderRadius: "6px", border: "1px solid #22223a" }}>
        <span style={{ fontSize: "11px", color: "#7a7a9a", fontWeight: "bold" }}>Reversible Lane (Street 3):</span>
        {REVERSIBLE_MODES.map(m => (
          <button key={m.id} onClick={() => { setReversibleMode(m.id); setOneWayPairs(false); }}
            style={reversibleMode === m.id && !oneWayPairs ? toggleOn : btn}>{m.label}</button>
        ))}
        
        <span style={{ width: 1, height: 16, background: "#333" }} />
        
        <button onClick={() => { setOneWayPairs(v => !v); setReversibleMode("none"); }}
          style={oneWayPairs ? toggleOn : btn}>
          {oneWayPairs ? "✓ " : ""}配對單行道 (Alternating One-Way Pairs)
        </button>
      </div>

      {/* Presets Controls */}
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", justifyContent: "center", alignItems: "center", background: "#111122", padding: "6px 12px", borderRadius: "6px", border: "1px solid #22223a" }}>
        <span style={{ fontSize: "11px", color: "#7a7a9a", fontWeight: "bold" }}>Scenario Presets:</span>
        <button onClick={() => {
          setSeed("42");
          setJitterOn(false);
          setMissingOn(false);
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center", fontSize: "11px", color: "#606078" }}>
          ...
          {expResults && (
            <button onClick={exportCSV} style={{ ...btn, background: "#1e293b", color: "#e2e8f0" }}>
              📥 Export CSV
            </button>
          )}
        </div>
